...
| زمان‌بند شلیک نمی‌کند / «کران نصب نیست» | [§۸.۱۱](#s8-11) |
...
**چک‌لیست چرخش (توصیهٔ رسمی این پروژه: چرخش کافی است، پاک‌سازی تاریخ لازم نیست)**

| # | اقدام | تأیید |
|---|---|---|
| ۱ | بله: در @BotFather بات → revoke توکن → توکن جدید | `curl -s "https://tapi.bale.ai/bot<NEW>/getMe"` جواب `ok:true` بدهد |
| ۲ | روبیکا: پنل توسعه‌دهندگان → باطل‌سازی → توکن جدید | `test_rubika.php` پاسخ `OK` بدهد |
| ۳ | سروش‌پلاس: اپ → «دستگاه‌های فعال» → پایان session وب | `login_soroush.js` ورود تازه + `smoke_test.sh --live` |
| ۴ | آی‌گپ: اپ → «نشست‌های فعال» → خاتمهٔ همه | `login_igap.js` ورود تازه |
| ۵ | توکن‌های جدید را فقط در `.env` بگذارید (نه در کد) | `bash setup_env.sh --check` سبز |
| ۶ | `SECURITY_KEY` تازه + هم‌مقدار کردن `.cron_key` | داشبورد با `?key=` جدید `200` بدهد |
| ۷ | تست نهایی چهار مقصد | `bash smoke_test.sh --live` → همه PASS |

> ✅ **چرا چرخش کافی است؟** توکن قدیمی پس از revoke دیگر هیچ دسترسی‌ای نمی‌دهد؛ بنابراین بودنِ آن در تاریخ Git بی‌اثر می‌شود. پاک‌سازی تاریخ (گام ۳) فقط «بهداشت اضافی» است و ریسک خودش را دارد (بازنویسی تاریخ + force push + ناسازگاری همهٔ clone ها).
>
> ⚠️ چرخش **بدون** به‌روزرسانی `.env` یعنی سیستم از کار می‌افتد: بلافاصله پس از revoke، `.env` را ویرایش و `bash smoke_test.sh` را اجرا کنید.
...
> 📌 **تصمیم این پروژه: این گام اجرا نمی‌شود.** چرخش توکن‌ها (گام ۱) دسترسی نشت‌شده را کاملاً بی‌اثر می‌کند؛ بازنویسی تاریخ ریسک عملیاتی بیشتری دارد تا سود. دستورهای زیر فقط برای مرجع نگه داشته شده‌اند.
>
...
### ۸.۱۱ زمان‌بند ساعت‌ها شلیک نمی‌کند / داشبورد می‌گوید «کران نصب نیست» <a id="s8-11"></a>

**علائم**

- پنل زمان‌بندی: بج «کران نصب نیست» یا heartbeat کهنه.
- ساعت‌ها ثبت شده‌اند ولی در وقت مقرر چیزی ارسال نمی‌شود.

**تشخیص**

```bash
cd /home/file/public_html/s
tail -20 logs/scheduler.log           # ۱) تیک اصلاً می‌آید؟ خطای PHP؟
crontab -l | grep scheduler_tick      # ۲) خط کران هست؟
bash scheduler_tick.sh                # ۳) اجرای دستی تیک — خروجی JSON می‌دهد
ACTION=schedule_list php cli_run.php  # ۴) اسلات‌ها، ساعت سرور و heartbeat
```
...
| علامت | علت | رفع |
|---|---|---|
| `logs/scheduler.log` اصلاً وجود ندارد | خط crontab ثبت نشده | `* * * * * /home/file/public_html/s/scheduler_tick.sh` را اضافه کنید (خط دقیق در خود داشبورد نمایش داده می‌شود) |
| `هیچ باینری PHP پیدا نشد` در scheduler.log | PHP در PATH کران نیست | `PHP_BIN=/opt/cpanel/ea-php83/root/usr/bin/php` در `.env` |
| heartbeat کهنه ولی اسلات ثبت است | کران با کاربر دیگری اجرا می‌شود (سطح دسترسی logs) | کران را با همان کاربر مالک برنامه (معمولاً `file`) تنظیم کنید |
| اسلات `skipped_busy` | در آن لحظه صف دیگری (دستی/کران قبلی) در حال اجرا بود | نیازی به کار نیست — عمدی است؛ پست جدید در چرخهٔ بعد می‌رود |
| ساعت‌ها با ساعت محلی شما نمی‌خوانند | منطقهٔ زمانی سرور فرق دارد | `SCHEDULE_TIMEZONE=Asia/Tehran` در `.env` (ساعت سرور در پنل نمایش داده می‌شود) |
| `schedule_runs` امروز `running` مانده | worker وسط اجرا بمیرد | عادی است؛ فردا دوباره شلیک می‌شود. علت مرگ را در §۸.۱۰ ببینید |

**پیشگیری** — بعد از نصب خط کران، یک دقیقه صبر کنید و ببینید بج پنل «زمان‌بند فعال» شد.
