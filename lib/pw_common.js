...
const APP_DIR = process.env.SYNC_APP_DIR || '/home/file/public_html/s';
const CHROMIUM_BIN = process.env.SYNC_CHROMIUM_BIN || '/usr/bin/chromium-browser';
...
function resolveChromium() {
    const envBin = process.env.SYNC_CHROMIUM_BIN;
...
async function launchBrowser(playwright, profileDir, viewport, log) {
    ...
    if (process.env.SYNC_USER_AGENT) opts.userAgent = process.env.SYNC_USER_AGENT;
    // برای ورود تعاملی روی ماشین دارای نمایشگر: SYNC_HEADED=1 node login_igap.js
    if (process.env.SYNC_HEADED === '1') opts.headless = false;
...
module.exports = {
    APP_DIR, CHROMIUM_BIN, resolveChromium,
    parseArgs, delay, ts,
