...
**دو action بازیابی (خارج از جریان عادی):** چون `sync_state` فقط `last_msg_id` را نگه می‌دارد و وضعیت هر پلتفرم را ثبت نمی‌کند، شکستِ جزئی یک پست (مثلاً بله/روبیکا موفق و سروش/آی‌گپ ناموفق) با جلو رفتن `last_msg_id` «پست دیده‌شده» می‌شود و دیگر به صف برنمی‌گردد. دو action برای جبران اضافه شده‌اند:

| action | ورودی | رفتار | `last_msg_id` |
|---|---|---|---|
| `resend` | `{ids:[…]}` یا `{from,to}` + `{only:[…]}` | scrape **تازه** (لینک رسانه امضاشده و زمان‌دار است) → دانلود رسانه → ارسال فقط به مقصدهای `only` | دست نمی‌خورد، مگر `advance:true` |
| `rewind` | `{id}` | `last_msg_id` را به شناسهٔ داده‌شده برمی‌گرداند تا پست‌ها دوباره در صف داشبورد ظاهر شوند | عقب می‌رود |

فیلتر `only` در `sync_single` هم پذیرفته می‌شود و هر دو از یک تابع مشترک `dispatchToPlatforms()` استفاده می‌کنند تا شکل پاسخ (`bale/rubika/soroush/igap` با `ok,info`) یکسان بماند؛ پلتفرم ردشده `{ok:null, info:"SKIPPED"}` می‌گیرد.

**پنل «ارسال اجباری» در داشبورد** یک کلاینتِ همین دو action است: با `list_posts` فهرست N پست آخر را می‌گیرد (بدون فیلتر `lastSeenId`) و سپس **به‌ازای هر پست یک درخواست `resend`** می‌فرستد. این تفکیک عمدی است: یک درخواست برای چند پست می‌تواند از سقف `max_execution_time`/ProxyTimeout بگذرد، اما هر درخواستِ تک‌پستی زیر کرانهٔ ۳۰۰ ثانیه می‌ماند — همان اصل micro-batching بخش ۸.۲، این بار برای مسیر بازیابی.
